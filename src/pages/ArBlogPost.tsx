...
export const ArBlogPostPage = () => {
  ...
  useEffect(() => {
    if (!slug) return;
    setLoading(true);
    getBlogPostBySlug(slug, "ar")
      .then(async (p) => {
        if (!p) {
          navigate("/ar/blog", { replace: true });
          return;
        }
        setPost(p);

        applySeo({
          title: `${p.titleAr} | مدونة تأكيد`,
          description: p.metaDescriptionAr || p.excerptAr || p.titleAr,
          path: `/ar/blog/${p.slugAr}`,
          language: "ar",
          type: "article",
          alternates: {
            ar: `/ar/blog/${p.slugAr}`,
          ...
          breadcrumb: [
            ...
            { name: p.titleAr, path: `/ar/blog/${p.slugAr}` },
          ],
          article: {
  ...
  const handleShare = async () => {
    ...
    if (navigator.share) {
      await navigator.share({ title: post?.titleAr, url });
    } else {
  ...
  const minutes = readingTime(post.contentAr || "");
      ...
      <div className="border-b border-gray-100 bg-gray-50 pt-24 pb-3">
        <div className="container mx-auto px-4">
          <nav className="flex items-center gap-1.5 text-xs text-gray-500">
            <Link to="/ar" className="hover:text-sky-700 transition-colors">
              الرئيسية
            </Link>
            <ChevronLeft className="w-3 h-3" />
            <Link to="/ar/blog" className="hover:text-sky-700 transition-colors">
              المدونة
            </Link>
            <ChevronLeft className="w-3 h-3" />
            <span className="text-gray-800 line-clamp-1">{post.titleAr}</span>
          </nav>
        </div>
      </div>
      ...
      <article className="container mx-auto px-4 py-12 max-w-4xl">
        ...
        <header className="mb-10">
          ...
          <div className="flex flex-wrap items-center gap-4 text-sm text-gray-500 mb-8">
            ...
            <button
              onClick={handleShare}
              className="flex items-center gap-1.5 bg-gray-100 hover:bg-gray-200 text-gray-600 px-3 py-1.5 rounded-lg transition-colors mr-auto"
            >
          ...
          {/* Excerpt */}
          {post.excerptAr && (
            <div className="bg-sky-50 border-r-4 border-sky-700 pr-5 py-4 rounded-l-xl mb-8">
        ...
        <div className="prose prose-lg prose-sky max-w-none
          ...
          prose-blockquote:border-r-4 prose-blockquote:border-sky-400 prose-blockquote:bg-sky-50 prose-blockquote:not-italic prose-blockquote:pr-6 prose-blockquote:rounded-l-xl
        ...
        {/* CTA Banner */}
        <div className="mt-14 bg-gradient-to-l from-sky-950 to-sky-800 rounded-2xl p-8 text-center">
          <h3 className="text-xl font-bold text-white mb-2">
            هل تريد تأسيس شركتك في دبي؟
          </h3>
          <p className="text-sky-200 text-sm mb-5">
            تواصل مع فريق تأكيد للحصول على استشارة مجانية
          </p>
          <Link
            to="/ar/contact-us"
        ...
        <section className="bg-gray-50 py-16" dir="rtl">
          <div className="container mx-auto px-4 max-w-5xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              {relatedPosts.map((rp) => (
                <Link key={rp.id} to={`/ar/blog/${rp.slugAr}`} className="group">
